...
const KNOWN_CLUBS = [
  { name: "Graf", company: "LICORELLA NOTTE SL", taxId: "B86377272", email: "", address: "Calle María de Molina, 50, 28006 Madrid, España", vatRate: 10, withholdingRate: 15, conceptTemplate: "SESIÓN DEL VALLE B2B ROSS GRAF", notes: "Histórico: 18/04/2026 200€." },
  { name: "Gabana", company: "PARTY LUXE, S.L.", taxId: "B87641015", email: "", address: "Calle María de Molina, 39, 28006 Madrid, España", vatRate: 10, withholdingRate: 15, conceptTemplate: "SESIÓN DEL VALLE B2B ROSS GABANA", notes: "Variantes reales: JUEVES / MIÉRCOLES." },
  { name: "Jimmys", company: "Primalia Espectáculos, SL", taxId: "B84360338", email: "", address: "C/ María de Molina, 39, 28006 Madrid, España", vatRate: 10, withholdingRate: 15, conceptTemplate: "SESIÓN DEL VALLE B2B ROSS JIMMYS", notes: "Histórico: 250€ por sesión." },
  { name: "Starlite Navidad", company: "STARLITE MUSIC GROUP SL", taxId: "B65900771", email: "", address: "C/ Antonio Herrero, 8 Pl 1 Puerta B, 28601 Marbella (Málaga)", vatRate: 10, withholdingRate: 15, conceptTemplate: "SESIÓN DEL VALLE B2B ROSS", notes: "3 sesiones de 200€." },
  { name: "Saint", company: "EXPANSION SISTEMA MA, SL", taxId: "B86184702", email: "", address: "C/ Madera, 7, Planta 1, Puerta Izq, 28004 Madrid", vatRate: 10, withholdingRate: 15, conceptTemplate: "Prestación de servicios DJ (Del Valle b2b Ross) en Saint Club", notes: "" },
  { name: "Bardot", company: "COMMODORO 1985, S.L", taxId: "B87072823", email: "", address: "Plaza República Argentina, 5, 28002 Madrid", vatRate: 10, withholdingRate: 15, conceptTemplate: "SESIÓN DEL VALLE B2B ROSS", notes: "Histórico: 3 sesiones de 300€ en septiembre." },
  { name: "Goose Panda", company: "Nahuel Mountain S.L", taxId: "B88475462", email: "", address: "C/ de los Hermanos Machado, 5 Bl 4 Bj B, 28660 Boadilla del Monte, Madrid", vatRate: 10, withholdingRate: 15, conceptTemplate: "SESIÓN DEL VALLE B2B ROSS PANDA", notes: "Histórico: 200€." },
  { name: "Blu", company: "Pentágono Escaleno, S.L", taxId: "B90488164", email: "", address: "Avenida de la República Argentina 35A, 1A, Sevilla 41011", vatRate: 10, withholdingRate: 15, conceptTemplate: "SESIÓN DEL VALLE B2B ROSS BLU", notes: "A veces con gastos de transporte aparte." },
  { name: "FITZ", company: "PRINCESA SOUNDS, SL", taxId: "B02853885", email: "", address: "Avenida de San Luis 95, 28003 Madrid", vatRate: 10, withholdingRate: 15, conceptTemplate: "Trabajo como DJ DEL VALLE B2B ROSS en eventos musicales en vivo, SALA FITZ", notes: "A veces con fecha y duración en la línea." },
  { name: "Teatro Barceló", company: "Teatro Barceló Madrid, SL", taxId: "B88471081", email: "", address: "Calle Barceló, 11, 28004 Madrid", vatRate: 10, withholdingRate: 15, conceptTemplate: "SESIÓN ROSS MAIN", notes: "Histórico Ross solo 200€." },
];
...
function bindEvents() {
  ...
  $("#add-selected-day-session-btn").addEventListener("click", () => openSessionDialog());
  $$(".segment[data-filter]").forEach((button) =>
    button.addEventListener("click", () => {
      ui.detailFilter = button.dataset.filter;
      $$(".segment[data-filter]").forEach((item) => item.classList.toggle("active", item === button));
      renderDetails();
    }),
  );
...
function renderStats() {
  ...
  const monthSessions = getSessionsForMonth(ui.currentMonth);
...
function renderCalendar() {
  ...
  $("#calendar-grid").innerHTML = buildCalendarDays(ui.currentMonth)
    .map((day) => {
      ...
      const dayBase = daySessions.reduce((sum, item) => sum + item.fee, 0);
      const isSelected = iso === ui.selectedDate;
      const isWeekSelected = formatDateInput(getWeekStart(day)) === ui.selectedWeekStart;
      const busyClass = daySessions.length >= 3 ? "busy-3" : daySessions.length === 2 ? "busy-2" : daySessions.length === 1 ? "busy-1" : "";
      return `
        <button class="calendar-day ${busyClass} ${day.getMonth() === ui.currentMonth.getMonth() ? "" : "outside"} ${iso === formatDateInput(now) ? "today" : ""} ${isSelected ? "selected" : ""} ${isWeekSelected ? "week-selected" : ""}" data-date="${iso}">
          ...
          <div class="calendar-chip-list">
            ${daySessions.slice(0, 3).map((session) => `<span class="chip">${escapeHtml(getClubName(session.clubId))}</span>`).join("")}
          </div>
...
function renderDetails() {
  const selectedDate = parseDateInput(ui.selectedDate);
  const allSessions = getSessionsForDate(ui.selectedDate);
  const sessions = ui.detailFilter === "all" ? allSessions : allSessions.filter((session) => session.participants === ui.detailFilter);
  $("#detail-title").textContent = formatHumanDate(selectedDate);
  $("#selected-day-number").textContent = String(selectedDate.getDate()).padStart(2, "0");
  $("#selected-day-label").textContent = new Intl.DateTimeFormat("es-ES", {
    weekday: "long",
    day: "2-digit",
    month: "long",
    year: "numeric",
  }).format(selectedDate);
  $("#selected-day-totals").textContent = `${allSessions.length} sesión${allSessions.length === 1 ? "" : "es"} · ${formatCurrency(allSessions.reduce((sum, item) => sum + item.fee, 0))}`;
    ...
    ? sessions
        ...
        .map((session) => `
            <article class="detail-card">
              <div class="item-row">
                <div>
                  <h4>${escapeHtml(getClubName(session.clubId))}</h4>
                  <p class="meta">${escapeHtml(session.title || "Sesión")}</p>
                </div>
                <span class="amount">${formatCurrency(session.fee)}</span>
              </div>
              <div class="detail-badges">
                <span class="pill">${getParticipantLabel(session.participants)}</span>
                ${session.invoiceId ? `<span class="pill paid">Facturada</span>` : `<span class="pill pending">Sin factura</span>`}
              </div>
          ...
          `)
        .join("")
    : `<div class="empty-state">No hay sesiones en este día para ese filtro.</div>`;
...
function renderUpcomingSessions() {
  const pending = [...state.invoices].filter((invoice) => !invoice.paid).sort((a, b) => a.date.localeCompare(b.date));
  $("#pending-invoices").innerHTML = pending.length
    ? pending
        .map((invoice) => `
          <article class="stack-card">
            <div class="item-row">
              <div>
                <h4>${escapeHtml(getClubName(invoice.clubId))}</h4>
                <p class="meta">Factura ${escapeHtml(invoice.number)} · ${formatHumanDate(parseDateInput(invoice.date))}</p>
              </div>
              <span class="amount">${formatCurrency(invoice.totalAmount)}</span>
            </div>
            <div class="detail-badges">
              <span class="pill pending">${invoice.items.length} sesión${invoice.items.length > 1 ? "es" : ""}</span>
            </div>
          </article>
        `)
        .join("")
...
function renderClubs() {
  $("#clubs-list").innerHTML = state.clubs.length
    ? state.clubs
        .map((club) => {
          const sessions = state.sessions.filter((session) => session.clubId === club.id);
          ...
          return `
            <article class="stack-card">
              ...
              ${club.conceptTemplate ? `<p class="meta">Concepto: ${escapeHtml(club.conceptTemplate)}</p>` : ""}
              <div class="club-tags">
...
function getInvoiceCandidates() {
  ...
  return state.sessions
    .filter((session) => session.clubId === clubId && !session.invoiceId && session.date.startsWith(month))
    .sort((a, b) => a.date.localeCompare(b.date));
}
...
function openSessionDialog(sessionId = null) {
  ...
  $("#session-title-input").value = defaultSessionTitle($("#session-club-select").value);
  ...
  $("#session-club-select").onchange = () => {
    if (!sessionId) $("#session-title-input").value = defaultSessionTitle($("#session-club-select").value);
  };
  $("#session-dialog").showModal();
}

function defaultSessionTitle(clubId) {
  const club = state.clubs.find((item) => item.id === clubId);
  return club?.conceptTemplate || `SESIÓN ${state.settings.brandName}`;
}
...
async function handleInvoiceSubmit(event) {
  ...
  if (!sessions.length) {
    alert(mode === "month" ? "No hay sesiones sin facturar para esa sala y ese mes." : "Selecciona al menos un día para facturar.");
    return;
  }
...
async function downloadInvoicePdf(invoice) {
  ...
  if (logo) doc.addImage(logo, "PNG", 205, 24, 180, 100);

  doc.setTextColor(16, 16, 16);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.text("CLIENTE", 42, 156);
  doc.text("FACTURA", 332, 156);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10.5);
  doc.text(`N.º de factura     ${invoice.number}`, 332, 176);
  doc.text(`Fecha factura      ${formatHumanDate(parseDateInput(invoice.date))}`, 332, 194);
  ...
  ].filter(Boolean).forEach((line, index) => doc.text(line, 42, 662 + index * 16));

  [
    club?.company || club?.name,
    club?.address || "",
    club?.taxId ? `CIF: ${club.taxId}` : "",
  ].filter(Boolean).forEach((line, index) => doc.text(line, 42, 176 + index * 16, { maxWidth: 220 }));

  doc.setFont("helvetica", "bold");
  doc.text("Conceptos", 42, 246);
  doc.text("Importe", 475, 246);
  doc.autoTable({
    startY: 258,
    head: [],
    body: invoice.items.map((item) => [formatShortDate(item.date), item.title || `Sesión ${s.brandName}`, formatEuroInline(item.fee)]),
    theme: "plain",
    styles: { fontSize: 10.5, cellPadding: { top: 4, right: 6, bottom: 4, left: 0 }, textColor: [16, 16, 16] },
    columnStyles: { 0: { cellWidth: 68 }, 1: { cellWidth: 320 }, 2: { halign: "right", cellWidth: 100 } },
  });

  const totalsY = doc.lastAutoTable.finalY + 30;
  doc.setFont("helvetica", "normal");
  doc.text("Base imponible", 340, totalsY);
  doc.text(formatEuroInline(invoice.baseAmount), 520, totalsY, { align: "right" });
  doc.text(`IVA ${club?.vatRate || 0}%`, 340, totalsY + 18);
  doc.text(formatEuroInline(invoice.vatAmount), 520, totalsY + 18, { align: "right" });
  doc.text(`Retención ${club?.withholdingRate || 0}%`, 340, totalsY + 36);
  doc.text(formatEuroInline(invoice.withholdingAmount), 520, totalsY + 36, { align: "right" });
  doc.setFont("helvetica", "bold");
  doc.setFontSize(14);
  doc.text("TOTAL", 340, totalsY + 66);
  doc.text(formatEuroInline(invoice.totalAmount), 520, totalsY + 66, { align: "right" });

  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  doc.text("Método de pago", 42, 600);
  doc.text(`${s.paymentMethod} al número de cuenta ${s.iban}`, 42, 618, { maxWidth: 460 });
  if (s.invoiceFooter) doc.text(s.invoiceFooter, 42, 748, { maxWidth: 520 });
...
function formatShortDate(value) {
  return new Intl.DateTimeFormat("es-ES", { day: "2-digit", month: "2-digit" }).format(parseDateInput(value));
}
